...
const gatt_attribute_t simple_profile_att_table[SP_IDX_NB] =
{
    ...
    [SP_IDX_SERVICE]                        =   {
        ...
        [SP_IDX_CHAR4_VALUE]                =   {
                                                    ...
                                                    GATT_PROP_WRITE | GATT_PROP_NOTI | GATT_PROP_READ,          /* Permissions */
 ...
 * @return  读请求的长度.
 */
static void sp_gatt_read_cb(uint8_t *p_read, uint16_t *len, uint16_t att_idx,uint8_t conn_id)
{
    switch (att_idx)
    {
       ...
       break;
				...
				case SP_IDX_CHAR4_VALUE:
            for (int i = 0; i < SP_CHAR4_VALUE_LEN; i++)
                sp_char4_value[i] = sp_char4_value[0] + i + 1;
            memcpy(p_read, sp_char4_value, SP_CHAR4_VALUE_LEN);	
            *len = SP_CHAR4_VALUE_LEN;
				break;
...
void test_gatt_notify(uint8_t conidx,uint8_t test)
{
        gatt_ntf_t ntf;
        ntf.conidx = conidx;
        ntf.svc_id = sp_svc_id;
        ntf.att_idx = SP_IDX_CHAR4_VALUE;
//        ntf.data_len = 1;
        //ntf.p_data = &test;
